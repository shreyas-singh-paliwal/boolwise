/* render.js — netlist tree (circuits.js) -> SVG string.
   Layered layout: nodes are leveled by longest path from the inputs (column
   = level), stacked top-to-bottom within their column. Because toNand()/
   toNor() share sub-nodes by object identity, walking the DAG that way means
   a shared gate/source is positioned and drawn exactly once, and every
   consumer just draws its own wire back to that one point (real fan-out). */

import { collectNodes } from './circuits';
...
function layout(root) {
  ...
  byLevel.forEach((col, lv) => {
    col.forEach((n, i) => pos.set(n, { level: lv, x: GEOM.pad + lv * GEOM.colW, y: GEOM.pad + i * GEOM.rowH }));
  });
...
/** Render a netlist tree (see circuits.js) as an SVG string. */
export function renderCircuitSVG(root) {
  ...
  nodes.forEach(n => {
    ...
    p._cy = midY;
    const cx = p.x + (n.t === 'NOT' ? GEOM.notW / 2 : GEOM.gateW / 2);
    gates += drawGate(n, cx, midY);
    p._outX = outX(n, cx);
    const pinXs = inXs(n, cx);
