/* parser.js — turns user input (expression | minterms/maxterms) into a truth table.
   Truth table shape used everywhere in this app:
     { vars: ['A','B',...], rows: [0|1|'X', ...] }   rows[i] = output for input i,
     where i's bits (MSB..LSB) correspond to vars[0..n-1]. Pure functions, no DOM. */
...
export function varsUsedIn(expr) {
  const s = new Set();
  for (const c of expr) if (/[A-Za-z]/.test(c)) s.add(c.toUpperCase());
  return [...s].sort();
}

export function truthFromExpression(expr, minVarCount) {
...
export function truthFromTerms(kind, termsText, dcText, minVarCount) {
  ...
  for (let i = 0; i < 2 ** n; i++) {
    ...
    else rows.push(termSet.has(i) ? 0 : 1);
  }
